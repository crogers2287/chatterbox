#!/usr/bin/env python3
"""
Chatterbox TTS API Server - Ultra Fast Edition
Maximized for 120+ it/s with direct CUDA graph inference
"""
...
def direct_t3_inference(
    ...
    """Direct T3 inference bypassing HuggingFace backend for maximum speed"""
    ...
    # Add start/stop tokens manually
    B = text_tokens.size(0)
    start_token = model.t3.hp.start_text_token
    stop_token = model.t3.hp.stop_text_token
    
    # Add start token if missing
    if text_tokens[0, 0] != start_token:
        text_tokens = torch.cat([
            torch.tensor([[start_token]], device=text_tokens.device),
            text_tokens
        ], dim=1)
    
    # Add stop token if missing
    if text_tokens[0, -1] != stop_token:
        text_tokens = torch.cat([
            text_tokens,
            torch.tensor([[stop_token]], device=text_tokens.device)
        ], dim=1)
    ...
    if model.conds is None:
        raise ValueError("No voice conditioning available")
    
    # Update exaggeration if needed
    if hasattr(model.conds.t3, 'emotion_adv') and model.conds.t3.emotion_adv[0, 0, 0] != exaggeration:
...
def get_compiled_model(model_func, backend="inductor"):
    """Get or create compiled version of model function"""
    key = f"{model_func.__name__}_{backend}"
    if key not in _compiled_models:
        logger.info(f"Compiling {model_func.__name__} with backend={backend}")
        _compiled_models[key] = torch.compile(
            model_func,
            backend=backend,
            mode="max-autotune",
            fullgraph=True,
...
async def load_model():
    ...
    try:
        ...
        try:
            ...
            # Try to compile the model's forward function
            try:
                model.t3.tfmr.forward = torch.compile(
                    model.t3.tfmr.forward,
                    backend="inductor",
                    mode="max-autotune",
                )
                logger.info("✓ Transformer forward compiled")
            except Exception as e:
                logger.warning(f"Could not compile transformer: {e}")
            
            # Try to compile text embedding
            try:
                model.t3.text_emb.forward = torch.compile(
                    model.t3.text_emb.forward,
                    backend="inductor",
                    mode="reduce-overhead",
                )
                logger.info("✓ Text embedding compiled")
            except Exception as e:
                logger.warning(f"Could not compile text embedding: {e}")
            
            # Try to compile speech embedding
            try:
                model.t3.speech_emb.forward = torch.compile(
                    model.t3.speech_emb.forward,
                    backend="inductor",
                    mode="reduce-overhead",
                )
                logger.info("✓ Speech embedding compiled")
            except Exception as e:
                logger.warning(f"Could not compile speech embedding: {e}")
            
            # Optimize s3gen
            try:
                model.s3gen.tf.forward = torch.compile(
                    model.s3gen.tf.forward,
                    backend="inductor",
                    mode="max-autotune",
                )
                logger.info("✓ S3Gen transformer compiled")
            except Exception as e:
                logger.warning(f"Could not compile S3Gen: {e}")
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting Chatterbox TTS API Server (Ultra Fast Edition)...")
    await load_model()
...
# FastAPI app
app = FastAPI(
    title="Chatterbox TTS API (Ultra Fast)",
    description="Ultra high-performance TTS API targeting 120+ it/s inference",
    version="3.0.0",
    lifespan=lifespan
)
...
async def root():
    """Root endpoint."""
    return {
        "message": "Chatterbox TTS API Server (Ultra Fast Edition)",
        "version": "3.0.0",
...
async def health_check():
    ...
    if model is not None:
        ...
        if hasattr(model.t3.tfmr.forward, '_compiled'):
            compile_status = "enabled"
        elif '_compiled_models' in globals() and len(_compiled_models) > 0:
...
async def synthesize_speech(
    ...
    try:
        ...
        # Use ultra-fast inference path
        with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
            with torch.no_grad():
                # Direct inference for maximum speed
                try:
                    # Try direct inference first
                    speech_tokens = direct_t3_inference(
                        model,
                        request.text,
                    ...
                    logger.warning(f"Direct inference failed, using standard path: {e}")
                    ...
                    t3_time = time.time() - t3_start
                    num_tokens = len(request.text) * 5  # Estimate
        ...
        # Calculate real T3 inference speed
        inference_speed = num_tokens / t3_time if t3_time > 0 else 0
        
        logger.info(f"Ultra-fast synthesis complete: {num_tokens} tokens in {t3_time:.2f}s = {inference_speed:.1f} it/s")
...
if __name__ == "__main__":
    port = int(os.environ.get("API_PORT", "6094"))
    uvicorn.run(
        "api_server_ultra_fast:app",
        host="0.0.0.0",
        port=port,
        reload=False,
        workers=1,
