/**
 * Enhanced Chatterbox TTS Store with Persistence
 * Zustand store with automatic state persistence and recovery capabilities
 */

import { create } from 'zustand';
import { voiceAPI } from '../voiceApi';
import { persist, PersistState } from './persistence';
...
export interface AppState extends PersistState {
  // Text management
  chunks: TextChunk[];
...
const DEFAULT_PARAMETERS: TTSParameters = {
  exaggeration: 0.5,
  temperature: 0.8,
  cfg_weight: 0.5,
  min_p: 0.05,
...
export const useStore = create<AppState>()(
  persist(
    persistConfig,
    (set, get) => ({
      // Persistence state (implemented by middleware)
      hasHydrated: false,
      rehydrate: async () => {},
      ...
      loadVoice: async (voiceId) => {
        ...
        if (voice.voice_file || voice.voiceReferenceData || (voice as any).voiceReferenceFile || voice.voiceReferenceUrl) {
          try {
        ...
        const parameters = voice.parameters || DEFAULT_PARAMETERS;
      ...
      recoverFromCrash: async () => {
        ...
        try {
          ...
          console.log('[Store] Recovered from crash successfully');
        } catch (error) {
          console.error('[Store] Failed to recover from crash:', error);
        }
      },
      ...
      saveRecoveryState: async () => {
        ...
        // Save current state to recovery storage
        try {
          // This will be handled by the persistence middleware automatically
          console.log('[Store] Recovery state saved automatically');
        } catch (error) {
          console.error('[Store] Failed to save recovery state:', error);
        }
      },
      ...
      clearRecoveryState: async () => {
        ...
        try {
          ...
          console.log('[Store] Recovery state cleared');
        } catch (error) {
          console.error('[Store] Failed to clear recovery state:', error);
        }
      },
    })
  )
...
export type { TextChunk, TTSParameters, SavedVoice, Session, BatchItem };

// Utility hooks for specific state slices
export const useChunks = () => useStore(state => state.chunks);
export const useParameters = () => useStore(state => state.parameters);
export const useSavedVoices = () => useStore(state => state.savedVoices);
export const useSessions = () => useStore(state => state.sessions);
export const useSystemStatus = () => useStore(state => state.systemStatus);

// Persistence utilities
export const usePersistence = () => useStore(state => ({
  hasHydrated: state.hasHydrated,
  rehydrate: state.rehydrate,
  getPersistedVersion: state.getPersistedVersion,
  clearPersistedState: state.clearPersistedState,
}));

// Recovery utilities
export const useRecovery = () => useStore(state => ({
  recoverFromCrash: state.recoverFromCrash,
  saveRecoveryState: state.saveRecoveryState,
  clearRecoveryState: state.clearRecoveryState,
}));