...
import { AutoSaveState, createAutoSaveSlice, createAutoSaveMiddleware } from './autoSaveIntegration';
import { storageManager } from '../storage';

// Extended state that includes recovery and auto-save slices
type EnhancedAppState = AppState & RecoverySlice & AutoSaveState;
...
// Create the store with persistence, recovery, and auto-save
  ...
  subscribeWithSelector(
    persist(
      (set, get, api) => {
        ...
        // Initialize auto-save middleware after store creation
        setTimeout(() => {
          createAutoSaveMiddleware(api);
        }, 100);
...
export const disposePersistenceMiddleware = () => {
  const storeApi = (useStore as any).getState?.() ? useStore as any : null;
